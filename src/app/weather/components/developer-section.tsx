  ...
  return (
    ...
    >
      ...
      <button
        ...
        className="w-full p-3.5 sm:p-5 flex items-center justify-between text-left hover:bg-surface-container-low/50 transition-colors cursor-pointer group focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-primary rounded-2xl"
      >
        <div className="flex items-center gap-2.5 sm:gap-3 min-w-0">
          <div className="w-8 h-8 sm:w-9 sm:h-9 rounded-xl bg-surface-container-high flex items-center justify-center text-primary shrink-0 group-hover:scale-105 transition-transform">
            <WeatherIcon name="code" className="w-4 h-4 sm:w-5 sm:h-5" />
          </div>
          <div className="min-w-0">
            <div className="flex items-center gap-2">
              <span className="font-display font-semibold text-on-surface text-sm sm:text-base">
                Developer API
              </span>
            ...
            <span className="text-[11px] sm:text-xs text-on-surface-variant block mt-0.5 line-clamp-1 sm:line-clamp-none">
      ...
      {/* Dropdown Content */}
      {isOpen && (
        <div className="px-3.5 pb-4 sm:px-6 sm:pb-6 pt-2 space-y-4 border-t border-white/[0.04] bg-surface-container-low/30 animate-[fade-in_0.2s_ease-out]">
          <p className="text-xs text-on-surface-variant leading-relaxed">
            Get instant weather forecasts in your projects with a simple GET request. Free for personal and developer use.
          </p>

          {/* cURL Snippet Box */}
          <div className="relative bg-surface-container-low rounded-xl p-3 sm:p-4 font-mono text-xs text-on-surface-variant overflow-x-auto shadow-inner border border-white/[0.04] touch-pan-x">
            <div className="flex items-center justify-between pb-2 text-[11px] font-semibold text-outline tracking-wider gap-2">
              <span className="truncate">TERMINAL / cURL REQUEST</span>
              <button
                type="button"
                onClick={handleCopy}
                className="text-primary hover:text-white transition-colors flex items-center gap-1.5 px-2.5 py-1 rounded bg-surface-container hover:bg-surface-container-high cursor-pointer font-sans shrink-0"
              >
                <WeatherIcon
            ...
            <pre className="pt-2 text-on-surface leading-relaxed select-all overflow-x-auto">
              <code>
                <span className="text-primary font-bold">curl</span> -X GET{" "}
                <span className="text-secondary">&quot;https://weather.maybesurya.dev/api/weather?city={activeCity || "Rudrapur"}&quot;</span>{" "}
                \<br />
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 sm:gap-3 pt-1 text-xs">
